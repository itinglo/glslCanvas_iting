...
void main() {
    ...
    float thickness=(0.65 * breathing + 0.084);//[0.1~0.2] //光環厚度 營造呼吸感
    ...
    float he = sin(u_time*0.43+4.0); 
    he = (0.001+abs(he)) * ((he>=0.0)?1.0:-1.0);
    float ra = 0.1 + 0.5*(0.5+0.5*sin(u_time*1.7)) + max(0.0,he-0.7);
    float d = sdBlobbyCross( uv, he ) - ra;
    */

    //float weight = smoothstep(0.1 , -0.2, uv.y);
    float uv_noise = gnoise(uv*10.0) * 0.5 + dir * 1.0  ; //(amplitude) * frequence
    //float uv_noise = gnoise(uv*10.0) * weight ; 
    // + 改變外框的位移 +內外有點不一樣
    // (*)改變整體 感覺好像celluar automata 
    float d = sdBlobbyCross( uv* 0.6 * pi * uv_noise + uv_noise*15.0+sin(u_time * .5), breathing); 


    //定質的話就不會動，但是如果也會動的話(上面的數值)那就會有動態
    float glow_circle = glow(d, strength, thickness); 
    gl_FragColor = vec4(vec3(glow_circle)*vec3(0.6196, 0.8353, 0.9922),1.0);
